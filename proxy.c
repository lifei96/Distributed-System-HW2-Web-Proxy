...
void construct_request(char *request, char *method, char *query,
        char *version, char *user_agent, char *host, char *connection,
...
void construct_request(char *request, char *method, char *query,
    ...
    while (strcmp(buf, "\r\n")) {
        if (!memcmp(buf, user_agent_name, sizeof(user_agent_name)) ||
            !memcmp(buf, host_name, sizeof(host_name)) ||
            !memcmp(buf, connection_name, sizeof(connection_name)) ||
            !memcmp(buf, proxy_connection_name, sizeof(proxy_connection_name))) {
            Rio_readlineb(&rio, buf, MAXLINE);
            continue;
        }
        printf("%s", buf);
